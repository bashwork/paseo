...
module Paseo
  class App < Sinatra::Base
    ...
    set :github_options, { :client_id    => ENV['PASEO_CLIENT_ID'],
                           :secret       => ENV['PASEO_SECRET'],
                           :scopes       => '',
    ...
    get '/seos/:id/?' do
      @seo = find_seo_or_view(params[:id])
    ...
    delete '/seos/:id/?' do
      ...
      if @seo = find_seo_or_view(params[:id])
        @user.remove_seo(@seo['id'])
      end
      request.xhr? ? "ok" : redirect('/')
    end

    get '/seos/:id/:view_id/?' do
      @seo = find_seo_or_view(params[:id])
      @view = find_seo_or_view(params[:view_id])
      @view_id = params[:view_id]
      @seo && @view ? mustache(:index) : not_found
    end

    get '/views/:id/?' do
      @view = find_seo_or_view(params[:id])
    ...
    def save_seo(params)
      id = sha(params.to_s)
      DB.save(:seos, id, params.merge(:id => id))
      @user.add_seo(id) if @user
      id
    end

    def find_seo_or_view(id)
      DB.find(:seos, id) || DB.find(:views, id)
    end
